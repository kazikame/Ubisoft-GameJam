...
public class BoardManager : MonoBehaviour
{
    ...
    public void PlaceWalls()
    {
        ...
        for (float x = xrangel; x < xranger; x++)
        {
            ...
            Instantiate(outerwall, new Vector3(x, 0f, zrangel), Quaternion.identity);

        }
        for(float z = zrangel; z < zranger; z++)
        {
            Instantiate(outerwall, new Vector3(xranger, 0f, z), Quaternion.identity);
        }
        for(float x = xranger; x > xrangel; x--)
        {
            Instantiate(outerwall, new Vector3(x, 0f, zranger), Quaternion.identity);
        }
        for(float z = zranger; z > zrangel; z--)
        {
            Instantiate(outerwall, new Vector3(xrangel, 0f, z), Quaternion.identity);
        }
